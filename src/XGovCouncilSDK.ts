...
import {
  APP_SPEC as COUNCIL_APP_SPEC,
  CouncilClient,
} from "./generated/CouncilClient";
...
import type { CouncilVote, CouncilMember } from "./types";

export class XGovCouncilSDK extends BaseSDK {
  public client: CouncilClient;
   ...
   */
  async getGlobalState(): Promise<{ registryAppId?: bigint; memberCount?: bigint } | null> {
    try {
   ...
   */
  async getVotes(proposalId: number | bigint): Promise<CouncilVote[]> {
    try {
   ...
   */
  async hasVoted(proposalId: number | bigint, memberAddress: string): Promise<boolean> {
   ...
   */
  async addMember(
    options: TransactionOptions & { memberAddress: string }
  ): Promise<boolean> {
