...
import {
  ...
  VotingState as GeneratedVotingState,
} from "./generated/ProposalClient";
import { ProposalStatusMap, ProposalStatus, ExtendedStatus, ProposalCategory, ProposalFocus, ProposalFundingType } from "./enums";
...
export class XGovProposalSDK extends BaseSDK {
   ...
   */
  async getSummary(): Promise<ProposalSummary | null> {
    try {
      ...
      return {
        ...
        fundingType: Number(state.fundingType ?? 0) as ProposalFundingType,
        status: Number(state.status ?? 0) as ProposalStatus,
        focus: (state.focus ?? 0) as ProposalFocus,
        fundingCategory: Number(state.fundingCategory ?? 0) as ProposalCategory,
   ...
   */
  async getMetadata(): Promise<ProposalMetadata | null> {
    try {
      const metadata = await this.algorand.app.getBoxValue(
        this.appId,
        metadataBoxName()
      );

      return JSON.parse(Buffer.from(metadata).toString()) as ProposalMetadata;
    } catch (e) {
   ...
   */
  async getDetails(): Promise<ProposalDetails | null> {
   ...
   */
  async getVotingState(): Promise<GeneratedVotingState | null> {
    try {
   ...
   */
  async getVoterAddresses(limit: number = 1000): Promise<string[]> {
   ...
   */
  async getVoterWeight(address: string): Promise<bigint | null> {
    try {
   ...
   */
  async getVotersInfo(addresses: string[]): Promise<VoterInfo[]> {
    const results = await Promise.allSettled(
      addresses.map((address) => this.getVoterWeight(address))
    );
  ...
  async open(
    ...
    try {
      ...
      const submissionFee = Math.trunc(
        Number((requestedAmount * publishingBps) / BigInt(10_000))
      );
  ...
  async updateMetadata(
    options: TransactionOptions & { data: UpdateMetadataInput }
  ): Promise<void> {
    ...
    try {
      ...
      const currentBoxSize = (
        await this.client.appClient.getBoxValue(_metadataBoxName)
      ).length;
   ...
   */
  async scrutinize(
    options: TransactionOptions & { proposerAddress: string }
  ): Promise<void> {
    ...
    try {
      const scrutinyTxn = (
        await this.client.createTransaction.scrutiny({
          sender,
          signer: wrappedSigner,
          args: {},
          appReferences: [this.registryAppId],
      ...
      console.warn(`While calling scrutiny(${this.appId}):`, (e as Error).message);
    }
  }
}
